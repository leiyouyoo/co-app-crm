...
import { PUBDataDictionaryService, PUBChargingCodeService, PUBCurrencyService } from '@co/cds';
import {
  CRMCustomerService,
  CRMQuoteEnquiryService,
  CRMQuoteEnquiryDto,
  CRMQuoteReplyService,
  CRMQuoteReplyDto,
} from 'apps/crm/app/services/crm';
import { CSPBookingService } from 'apps/crm/app/services/csp';
import { Observable } from 'rxjs';
import { FreightMethodType } from 'apps/crm/app/shared/types/booking/FreightMethodType';

@Component({
  selector: 'booking-createquotes',
  templateUrl: './bookingcreatequotes.component.html',
  styleUrls: ['./bookingcreatequotes.component.less'],
})
export class bookingCreatequotesComponent implements OnInit {
  ...
  constructor(
    ...
    private crmQuoteEnquiry: CRMQuoteEnquiryService,
  ...
  @Output() isQuoteStatus = new EventEmitter<any>();
  ...
  GetCustomerBindUserForCRM(Id: string) {
    this.cspBookingService
      ...
      .subscribe((res) => {
        ...
        this.selCustomer(res.customerId);
      });
  }
  ...
  ngSubmit() {
    ...
    if (!this.isCreate) this.quoteReplys.quoteEnquiryId = this._quoteinfo.id;
    ...
    if (this.isCreate) {
      ...
      this.crmQuoteEnquiry.create(this._quoteinfo).subscribe(
        (c) => {
